...
//
// Signal-handler safety: this runs in async-signal context, so it must NOT
// take g_state_mutex (the trapped thread may already hold it — device-proven
// deadlock in electerm-harmony 2026-08-28) and must NOT call the NAPI exit
// callback (threadsafe_function is not signal-safe). We only use SafeWrite
// + a lock-free store to g_running; the ArkTS side observes the failure via
// the next getStatus/isRunning poll and the boot-log tail.
static void CrashMarkerHandler(int sig, siginfo_t* si, void* ctx) {
  (void)si;
  (void)ctx;
  if (sig == SIGABRT) {
    SafeWrite("[embed] fatal: signal SIGABRT — parking node thread\n");
  } else if (sig == SIGSEGV) {
    SafeWrite("[embed] fatal: signal SIGSEGV — parking node thread\n");
  } else if (sig == SIGBUS) {
    SafeWrite("[embed] fatal: signal SIGBUS — parking node thread\n");
  } else if (sig == SIGILL) {
    SafeWrite("[embed] fatal: signal SIGILL — parking node thread\n");
  } else if (sig == SIGFPE) {
    SafeWrite("[embed] fatal: signal SIGFPE — parking node thread\n");
  } else {
    SafeWrite("[embed] fatal: signal — parking node thread\n");
  }
  long tid = (long)syscall(__NR_gettid);
  if (g_node_tid > 0 && tid == (long)g_node_tid) {
    // Node's thread crashed — keep the app alive. Lock-free: a plain store
    // is sufficient for the crash path; normal paths keep using the mutex.
    g_running = false;
    for (;;) pause();
  }
  signal(sig, SIG_DFL);
