 ...
 *
 * Relative paths always use `/` (the rawfile lookup on-device joins with
 * `/`), while filesystem access splits them back into segments so this works
 * on Windows as well.
 */
async function listNodeFiles(dir: string): Promise<string[]> {
  ...
  const walk = async (sub: string): Promise<void> => {
    let entries: string[] = [];
    try {
      entries = await readdir(sub.length ? join(dir, ...sub.split('/')) : dir);
    } catch (e) {
      return;
    }
...
export async function applyModuleOverrides(env: HookEnv): Promise<void> {
  ...
  if (harmony.permissions?.length && (await pathExists(moduleJson))) {
    ...
    // Deduplicate: never insert a permission that is already declared, so
    // repeated `cap sync` runs stay idempotent.
    const missing = harmony.permissions.filter((p) => !new RegExp(`"name"\\s*:\\s*"${p}"`).test(original));
    if (missing.length === 0) {
      return;
    }
    ...
    if (rp.test(original)) {
      ...
      const moduleOpen = /("module"\s*:\s*\{)/;
      if (moduleOpen.test(original)) {
