...
import type { NodePlugin, NodeStartResult, NodeStatus, NodeInfo, NodeLog, NodeApiOptions, NodeApiResult } from './definitions';
...
export class NodeWeb implements NodePlugin {
  ...
  callApi(): Promise<NodeApiResult> {
    return Promise.reject(new Error('no embedded node on web'));
  }
